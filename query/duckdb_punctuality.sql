        ...
        FROM
            (
                ...
                FROM
                    ...
                    LEFT JOIN qdaba.linie_lb llb ON llb.id_t_linie = l.id_t_linie AND p_2.betriebstag BETWEEN CAST(substring(llb.gueltigkeit, 2, 10) AS DATE) AND CAST(nullif(substring(llb.gueltigkeit, 13, length(llb.gueltigkeit) - 13), '') AS DATE)
                    LEFT JOIN qdaba.linienbuendel lb ON lb.id_t_lb = llb.id_t_lb AND p_2.betriebstag BETWEEN CAST(substring(lb.gueltigkeit, 2, 10) AS DATE) AND CAST(nullif(substring(lb.gueltigkeit, 13, length(lb.gueltigkeit) - 13), '') AS DATE)
            ) p_1
    ) p
