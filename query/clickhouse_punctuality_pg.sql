...
FROM
    (
        ...
        FROM
            puenkt_pg p_2
                LEFT JOIN puenkt_fahrt_pg pf ON pf.id_t_puenkt_fahrt = p_2.id_t_puenkt_fahrt
                LEFT JOIN linie_pg l ON l.id_t_linie = p_2.id_t_linie_bav
                LEFT JOIN linie_lb_pg llb ON llb.id_t_linie = l.id_t_linie AND p_2.betriebstag BETWEEN toDate(substring(llb.gueltigkeit, 2, 10)) AND toDateOrNull(substring(llb.gueltigkeit, 13, length(llb.gueltigkeit) - 13))
                LEFT JOIN linienbuendel_pg lb ON lb.id_t_lb = llb.id_t_lb AND p_2.betriebstag BETWEEN toDate(substring(lb.gueltigkeit, 2, 10)) AND toDateOrNull(substring(lb.gueltigkeit, 13, length(lb.gueltigkeit) - 13))
        ) p_1) p JOIN puenkt_kat_pg pk ON CAST(p.id_anwendungsfall AS String) = CAST(p.id_anwendungsfall AS String)
