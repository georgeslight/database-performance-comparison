create view v_dpm_vorberechnet_neu
            (puenkt_kat, id_t_import, id_anwendungsfall, id_mvu, id_linie_tu, betriebstag, id_messpunkt, id_an_ab,
             id_t_linie_bav, qrelevant, id_verspcode, anz_puenkt_tu, anz_puenkt_bav, anz_mess_tu, anz_mess_bav, go_nr,
             id_t_lb, direction_id)
as
...
FROM (SELECT p_1.id_t_import,
      ...
      FROM (SELECT p_2.id_t_import,
            ...
            FROM <schemaName>.puenkt p_2
                     ...
                     LEFT JOIN <schemaName>.linie_lb llb
                               ON llb.id_t_linie = l.id_t_linie AND llb.gueltigkeit @> p_2.betriebstag
                     LEFT JOIN <schemaName>.linienbuendel lb
                               ON lb.id_t_lb = llb.id_t_lb AND lb.gueltigkeit @> p_2.betriebstag) p_1) p
         JOIN <schemaName>.puenkt_kat pk ON p.id_anwendungsfall::text = pk.id_anwendungsfall::text AND
