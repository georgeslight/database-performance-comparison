...
ATTACH 'dbname=qdaba user=postgres host=192.168.100.103 port=5432 password=oracle' AS pg (TYPE POSTGRES, READ_ONLY);
...
INSERT INTO qdaba.anwendungsfall SELECT * FROM pg.qdaba.anwendungsfall;
...
INSERT INTO qdaba.verkehrstraeger SELECT * FROM pg.qdaba.verkehrstraeger;
...
INSERT INTO qdaba.mvu SELECT * FROM pg.qdaba.mvu;
...
INSERT INTO qdaba.kanton SELECT * FROM pg.qdaba.kanton;
...
INSERT INTO qdaba.verkehrsmittel_kategorie SELECT * FROM pg.qdaba.verkehrsmittel_kategorie;
...
INSERT INTO qdaba.import SELECT * FROM pg.qdaba.import;

CREATE OR REPLACE TABLE qdaba.bhf (
...
SELECT
    ...
    CAST(SUBSTRING(gueltigkeit, 2, POSITION(',' IN gueltigkeit) - 2) AS DATE) AS gueltigkeit_start,
    CAST(SUBSTRING(gueltigkeit, POSITION(',' IN gueltigkeit) + 1, LENGTH(gueltigkeit) - POSITION(',' IN gueltigkeit) - 1) AS DATE) AS gueltigkeit_end,
    go_nr,
    gemeinde
FROM pg.qdaba.bhf;


CREATE OR REPLACE TABLE qdaba.linie (
...
SELECT
    ...
    CAST(SUBSTRING(gueltigkeit, 2, POSITION(',' IN gueltigkeit) - 2) AS DATE) AS gueltigkeit_start,
    CASE
        WHEN LENGTH(gueltigkeit) > 13 THEN
            CAST(SUBSTRING(gueltigkeit, POSITION(',' IN gueltigkeit) + 1, LENGTH(gueltigkeit) - POSITION(',' IN gueltigkeit) - 1) AS DATE)
        ELSE
            NULL
...
INSERT INTO qdaba.puenkt_fahrt SELECT * FROM pg.qdaba.puenkt_fahrt;
...
INSERT INTO qdaba.puenkt_kat SELECT * FROM pg.qdaba.puenkt_kat
...
CREATE table qdaba.puenkt (
    ...
    UNIQUE(id_anwendungsfall, id_mvu, go_nr, id_t_linie_bav, betriebstag, id_fahrt, fahrt_halt_lauf, id_messpunkt, id_an_ab)
);
INSERT INTO qdaba.puenkt SELECT * FROM pg.qdaba.puenkt;